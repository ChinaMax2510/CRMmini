...
function goView(){
	var id=$("#id").val();
	var url = "${pageContext.request.contextPath}/com/Company_showShareViewOne.do?id="+id;
...
function goCancel(){	
	...
	var url = "${pageContext.request.contextPath}/com/Company_showShareCancel.do?id="+id;
	forward(url);
}
...
<body>
<s:form name="ActionForm" method="post" action="Company_updateShareSetOne.do"  namespace="/com">
				...
				<fieldset style='padding:5px;clear:left;'>
						 <legend>${sysUserGroup.name}</legend>
	                       <c:if test="${! empty requestScope.sysUsers}">
		                       <c:forEach items="${requestScope.sysUsers}" var="User">  
		                       <!-- group首字母要小写，不然识别不出 --> 
		                          <c:if test="${User.group.id==sysUserGroup.id }">
			                          <div>
									     <input type='checkbox' class='checkbox' name='uid' value='${User.id}' id='uid${User.id}'  onclick='do_select(this.id)' >${User.cnname}
									  </div>&nbsp;
								  </c:if>
		                       </c:forEach>
	                       </c:if>
