 ...
 function do_share(){
	   ...
	   if(count==0){
		...
		if(count==1){  //选中一个客户的情况下
		   ...
		   OpenWin("${pageContext.request.contextPath}/com/Company_shoeShareOne.do?id="+ids,'',500,400)
		   //OpenWin("${pageContext.request.contextPath}/crm/customer/base/shareSetOne.jsp",'',500,400);
		}else{  //选中多个的情况下
			alert("此系统版本暂时不支持多客户操作！^…^ ")
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabForm">
  ...
  <tr>
	  <td colspan="2">
<!-- 基本查询的表单-->
<s:form name="form1" method="post" action="Company_list.do" namespace="/com">
<table width="100%" border="0" cellspacing="0" cellpadding="0" name="base" id="base">
	...
	<tr>
		...
		<td>客户等级：</td>
	<td>
		<s:if test="%{#request.companygrade!=null}">
		  <s:select list="%{#request.companygrade}" id='grade' name='grade' cssStyle="width:110px"
...
<div class="control">
	<button type='button' class='button' onMouseOver="this.className='button_over';" onMouseOut="this.className='button';"  
	     onClick="forward('${pageContext.request.contextPath}/com/Company_add.do')">
...
<div class="border">
<s:form method="post" action="Company_delete.do" namespace="/com">
		...
		<s:iterator value="#request.companys"  var="company">
			<tr>
		    	<td>
		  	    ...
		  	    <td>
		  	       <a href="${pageContext.request.contextPath}/com/Company_edit.do?id=<s:property value="#company.id"/>"><s:property value="#company.name"/></a>
		  	    </td>
...
<script type="text/javascript">
	function goChangePerson() {
	  ...
	  var url = "${pageContext.request.contextPath}/com/Company_changePersonView.do?ids="+ids;
    ...
    function gonextTouchTime(){
	  ...
	  var ids="";
	   //遍历所有的复选框
	   $("input[type='checkbox'][name='ids']").each(function(index,data){//逐个遍历复选框
	 ...
	 OpenWin("${pageContext.request.contextPath}/com/Company_nextTouchTimeview.do?ids="+ids,'',420,300);
	}
