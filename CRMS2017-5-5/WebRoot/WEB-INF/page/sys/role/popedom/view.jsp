...
<body>
<s:form name="ActionForm" method="post" action="sysRoleAction_updatePopdeom.do" namespace="/sys">
<s:hidden name="roleId" value="%{#request.roles.id}" />
<br/>
<div class="control">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#6A82A8">
	<tr>
		<td align="center" height="25"><span style="color:#FFFFFF; font-weight:bold">操作权限组：${roles.name}</span></td>
	</tr>
</table>
<div class="border" style="padding:3px">
<c:if test="${! empty requestScope.sysPopedoms}"><!-- 功能显示 -->
			...
			<c:if test="${sysPopedomSup.id.popedomModule==sysPopedomSup.id.popedomPrivilege}">
				<fieldset style='padding:5px;clear:left;'>
				    ...
				    <!-- 遍历权限组包含的权限 处理父的复选框回显 -->
						...
						<!-- 如果权限表和菜单显示表有对应（两个主键popedomModule，popedomPrivilege有对应 ）说明被选择-->
	                          <c:if test="${sysPopedomSup.id.popedomModule==sysPopedomPrivile.ids.popedomModule
	                                       &&sysPopedomSup.id.popedomPrivilege==sysPopedomPrivile.ids.popedomPrivilege}">
	                              <c:set value="checked" var="xx" scope="page"/>
	                          </c:if>	
					 	</c:forEach>
				
					    <legend>
					     <input type='checkbox' class='checkbox' name='popedomModule'    ${xx}
				 	...
				 	<c:forEach items="${requestScope.sysPopedoms}" var="sysPopedomSub">   
				 	    <c:if test="${sysPopedomSup.id.popedomModule==sysPopedomSub.id.popedomModule
						    ...
						    <!-- 遍历权限组包含的权限 处理子的复选框回显 -->
							   <c:forEach items="${requestScope.sysPopedomPriviles}" var="sysPopedomPrivile">
			                          <c:if test="${sysPopedomSub.id.popedomModule==sysPopedomPrivile.ids.popedomModule
			                                       &&sysPopedomSub.id.popedomPrivilege==sysPopedomPrivile.ids.popedomPrivilege}">
